...
A fairly simple yet effective Pagination component, showing a previous and next page and indicating to the user the current page it is on and the total pages that are available. This is a great component to show on pages generated by a search request for example.

**Some (accessibility) features of the Pagination**:
- Uses `aria-label`'s to tell the user whether they will go to the previous or next page
